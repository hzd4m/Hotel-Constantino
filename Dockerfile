...
# docker build -t hotel_constantino .
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name hotel_constantino hotel_constantino
...
RUN mkdir -p /rails/tmp/pids /rails/tmp/cache /rails/tmp/sockets && \
...
# Start server (Render will pass PORT env var)
EXPOSE 3000
